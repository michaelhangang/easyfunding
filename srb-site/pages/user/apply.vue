...
export default {
  data() {
    return {
      active: null, // Step
      borrowInfoStatus: null, // Approval status
      //借款申请
      borrowInfo: {
        borrowYearRate: '12',
      },
      borrowAmount: 0, // Borrow amount
      submitBtnDisabled: false,
      returnMethodList: [], // Return method
      moneyUseList: [], // Money use
    }
  },
  mounted() {
  ...
  watch: {
    'borrowInfo.amount'(value) {
      if (value > this.borrowAmount) {
        let _this = this
        this.$alert('Not enough borrow amount!', {
          type: 'error',
          callback() {
